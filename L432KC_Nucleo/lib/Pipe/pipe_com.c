...
void PIPE_COM_IRQHANDLER() {
  ...
  if (LL_USART_IsActiveFlag_IDLE(PIPE_COM)) {
    ...
    LastData = PIPE_TO_LUOS_BUFFER_SIZE -
               LL_DMA_GetDataLength(P2L_DMA, P2L_DMA_CHANNEL);
    if (LastData == 0) {
...
void L2P_DMA_IRQHANDLER() {
  ...
  // check if we receive an IDLE on usart3
  // check if we receive an IDLE on usart3
  if ((L2P_DMA_TC(L2P_DMA) != RESET) &&
      (LL_DMA_IsEnabledIT_TC(L2P_DMA, L2P_DMA_CHANNEL) != RESET)) {
