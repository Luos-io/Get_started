    ...
    for (uint8_t i = 0; i < PIPE_TO_LUOS_MAX_TASK; i++) {
      if (P2LTask[i].data_pt == 0) {
        ...
        if (P2LBuffer_PrevStartData < PositionLastData) {
          ...
          P2LTask[i].size =
              (((PIPE_TO_LUOS_BUFFER_SIZE)-P2LBuffer_PrevStartData) +
               PositionLastData);
        }
...
static uint8_t PipeBuffer_P2LTaskNeedClear(uint16_t PositionLastData) {
  ...
  if (P2LTaskID > 0) {
    if (((&P2L_Buffer[PositionLastData] > (P2LTask[0].data_pt)) &&
         (&P2L_Buffer[PositionLastData] < (P2LTask[0].end))) ||
        ((&P2L_Buffer[PositionLastData] < (P2LTask[0].data_pt)) &&
         (&P2L_Buffer[PositionLastData] > (P2LTask[0].end)))) {
      result = true;
      PipeBuffer_ClearP2LTask();
    }
