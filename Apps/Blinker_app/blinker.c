...
typedef enum { BLINKER_APP = LUOS_LAST_TYPE } App_type_t;
...
void Blinker_Init(void) {
  ...
  blinker_app =
      Luos_CreateService(Blinker_MsgHandler, BLINKER_APP, "blinker", revision);
...
void Blinker_Loop(void) {
  ...
  if ((my_time < Luos_GetSystick()) && (control_app.flux == PLAY)) {
    ...
    if (id_led > 0) {
      ...
      led_msg.header.cmd =
          IO_STATE; // We are specifying an IO state (on or off)
      led_msg.header.target_mode =
          IDACK; // We are asking for an acknowledgement

      led_msg.header.size =
          sizeof(char); // Our message only contains one character, the IO state
